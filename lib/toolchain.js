module.exports = Object.create({
        ...
        processRequest: function(req) {
                ...
                // Hack: Translate buffer to string, extract char code
                tmpByte = tmpSlice.toString('binary', 0, 1).charCodeAt(0);

                // qr
                // 1 bit
                query.header.qr = this.utils.sliceBits(tmpByte, 0, 1);

                // opcode
                // 0 = standard, 1 = inverse, 2 = server status, 3-15 reserved
                // 4 bits
                query.header.opcode = this.utils.sliceBits(tmpByte, 1, 4);

                // authorative answer
                // 1 bit
                query.header.aa = this.utils.sliceBits(tmpByte, 5, 1);

                // truncated
                // 1 bit
                query.header.tc = this.utils.sliceBits(tmpByte, 6, 1);

                // recursion desired
                // 1 bit
                query.header.rd = this.utils.sliceBits(tmpByte, 7, 1);
                ...
                // recursion available
                // 1 bit
                query.header.ra = this.utils.sliceBits(tmpByte, 0, 1);

                // reserved 3 bits
                // rfc says always 0
                query.header.z = this.utils.sliceBits(tmpByte, 1, 3);
                ...
                // 6-15 reserved
                // 4 bits
                query.header.rcode = this.utils.sliceBits(tmpByte, 4, 4);
        ...
        createResponse: function(query, records) {
                /*
                 * Step 1: find record associated with query
                 */
                var results = this.findRecords(query.question.qname, 1, void 0, records);
                 ...
                 */
                return this.buildResponseBuffer(response);
        },

        buildResponseBuffer: function(response) {
                ...
                this.utils.numToBuffer(buf, 4, response.header.qdcount, 2);

                this.utils.numToBuffer(buf, 6, response.header.ancount, 2);
                this.utils.numToBuffer(buf, 8, response.header.nscount, 2);
                this.utils.numToBuffer(buf, 10, response.header.arcount, 2);
                ...
                for (var i = 0; i < response.rr.length; i++) {
                        ...
                        var tmpBuf = Buffer(buf.length + response.rr[i].qname.length + 14); tmpBuf.fill(0);

                        buf.copy(tmpBuf, 0, 0, buf.length);

                        response.rr[i].qname.copy(tmpBuf, rrStart, 0, response.rr[i].qname.length);
                        this.utils.numToBuffer(tmpBuf, rrStart + response.rr[i].qname.length, response.rr[i].qtype, 2);
                        this.utils.numToBuffer(tmpBuf, rrStart + response.rr[i].qname.length + 2, response.rr[i].qclass, 2);

                        this.utils.numToBuffer(tmpBuf, rrStart + response.rr[i].qname.length + 4, response.rr[i].ttl, 4);
                        this.utils.numToBuffer(tmpBuf, rrStart + response.rr[i].qname.length + 8, response.rr[i].rdlength, 2);
                        this.utils.numToBuffer(tmpBuf, rrStart + response.rr[i].qname.length + 10, response.rr[i].rdata, response.rr[i].rdlength); // rdlength indicates rdata length
        ...
        findRecords: function(qname, qtype, qclass, records) {
                ...
                if (qclass === undefined || qclass === 1) {
                        ...
                        // Only internet class records supported
                ...
                var domain = this.utils.qnameToDomain(qname);
                ...
                if (qtype === '*') {
                        ...
                        if (records[domain] && records[domain][qclass] && records[domain][qclass][qtype]) {
